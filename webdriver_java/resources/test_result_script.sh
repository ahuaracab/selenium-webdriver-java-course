...
TEST_RESULTS_STRING=$(grep "<testng-results" "${TEST_RESULTS_LOCATION}/testng-results.xml")
...
{
  echo " gha.maventest.ignored=$(echo "${TEST_RESULTS_STRING}" | awk -F'"' '{ print $2 }')"
  echo " gha.maventest.total=$(echo "${TEST_RESULTS_STRING}" | awk -F'"' '{ print $4 }')"
  echo " gha.maventest.passed=$(echo "${TEST_RESULTS_STRING}" | awk -F'"' '{ print $6 }')"
  echo " gha.maventest.failed=$(echo "${TEST_RESULTS_STRING}" | awk -F'"' '{ print $8 }')"
  echo " gha.maventest.skipped=$(echo "${TEST_RESULTS_STRING}" | awk -F'"' '{ print $10 }')"
} >> "$BUILDEVENT_FILE"
